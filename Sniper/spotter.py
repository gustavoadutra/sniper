...
from Interface import variables as var
...
SPOOTER_OK = dpg.add_bool_value(default_value=False, parent=interface.values_registry)
...
def mouse_l_button(sender, data, user):
    ...
    interface.print_callback('Tracking ' + str(dpg.get_value(L_BUTTON)))
...
def run_spotter():
    ...
    if spotter_status:
        ...
        if dpg.get_value(L_BUTTON):
            if dpg.get_value(KEY_G):
                try:
                    ...
                    dpg.configure_item(interface.SPOTTER, default_value='SPOTTER: ACTIVATED')
                except:
        ...
        if dpg.get_value('KEY_A'):
            ...
            cv2.line(spotter_texture, (center_x_shooter, 0), (center_x_shooter, spotter_texture.shape[0]), (var.BGR[7]),
                     2)
            cv2.line(spotter_texture, (0, center_y_shooter), (spotter_texture.shape[1], center_y_shooter), (var.BGR[7]),
                     2)
...
def init_spotter():
    ...
    try:
        ...
        img_spotter = dpg.add_raw_texture(parent=interface.textures_registry, height=h, width=w,
                                          default_value=spotter_texture, format=dpg.mvFormat_Float_rgb)
        dpg.configure_item('img_spotter', texture_tag=img_spotter)
